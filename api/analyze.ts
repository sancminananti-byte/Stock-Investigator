...
// Vercel에서 이 파일을 자동으로 서버리스 함수로 배포합니다.
// 이 함수는 Gemini API에 대한 보안 프록시 역할을 합니다.
export default async function handler(req: any, res: any) {
  ...
  if (!ticker || typeof ticker !== 'string') {
    return res.status(400).json({ error: '요청 본문에 "ticker" 문자열이 필요합니다.' });
  }
  
  const apiKey = process.env.API_KEY;
  if (!apiKey) {
    console.error("API_KEY environment variable not set on server.");
    // 클라이언트에게는 상세한 오류 정보를 노출하지 않습니다.
  ...
  try {
    ...
    if (!jsonMatch) {
      throw new Error("모델 응답에서 유효한 JSON 객체를 찾을 수 없습니다.");
    }
    
    const analysisData = JSON.parse(jsonMatch[0]) as StockAnalysis;
    
    return res.status(200).json({ analysisData, sourcesData });

  } catch (error) {
    console.error("Error in serverless function:", error);
    if (error instanceof Error && (error.message.includes("API key not valid") || error.message.includes("API_KEY_INVALID"))) {
      return res.status(500).json({ error: "서버에 설정된 Gemini API 키가 유효하지 않습니다." });
    }
    return res.status(500).json({ error: "AI 분석 데이터를 가져오는 데 실패했습니다." });
  }
}
